(globalThis.TURBOPACK = globalThis.TURBOPACK || []).push(["static/chunks/app_admin_asignar_page_tsx_3596df75._.js", {

"[project]/app/admin/asignar/page.tsx [app-client] (ecmascript)": ((__turbopack_context__) => {
"use strict";

var { g: global, __dirname, k: __turbopack_refresh__, m: module } = __turbopack_context__;
{
// app/admin/asignar/page.tsx
__turbopack_context__.s({
    "default": (()=>AsignarPromosPage)
});
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/next/dist/compiled/react/jsx-dev-runtime.js [app-client] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/next/dist/compiled/react/index.js [app-client] (ecmascript)");
var __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$clsx$2f$dist$2f$clsx$2e$mjs__$5b$app$2d$client$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/node_modules/clsx/dist/clsx.mjs [app-client] (ecmascript)");
;
var _s = __turbopack_context__.k.signature();
'use client';
;
;
/* -------------------------------------------------------------------------- */ /*                           Helpers localStorage                             */ /* -------------------------------------------------------------------------- */ const getProductos = ()=>JSON.parse(localStorage.getItem('kwikemart_productos') || '[]');
const getPromos = ()=>JSON.parse(localStorage.getItem('kwikemart_promos') || '[]');
const saveAsignacion = (productoId, promoId)=>{
    const asign = JSON.parse(localStorage.getItem('kwikemart_asignaciones') || '[]');
    localStorage.setItem('kwikemart_asignaciones', JSON.stringify([
        ...asign,
        {
            productoId,
            promoId
        }
    ]));
};
function AsignarPromosPage() {
    _s();
    const [productos, setProductos] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])([]);
    const [promos, setPromos] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])([]);
    const [productoId, setProductoId] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])('');
    const [promoId, setPromoId] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])('');
    const [msg, setMsg] = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useState"])({});
    /* Carga inicial */ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useEffect"])({
        "AsignarPromosPage.useEffect": ()=>{
            setProductos(getProductos());
            setPromos(getPromos());
        }
    }["AsignarPromosPage.useEffect"], []);
    const handleSubmit = (e)=>{
        e.preventDefault();
        setMsg({});
        if (!productoId || !promoId) return setMsg({
            err: 'Selecciona un producto Y una promoción.'
        });
        saveAsignacion(+productoId, +promoId);
        setMsg({
            ok: '¡Promoción asignada correctamente!'
        });
        setProductoId('');
        setPromoId('');
    };
    /* ------------------------------ UI ---------------------------------- */ return /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("section", {
        className: "relative min-h-[700px] p-6 pb-22",
        children: /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
            className: "mx-auto mt-12 flex max-w-xl flex-col items-center rounded-lg bg-white p-8 shadow-lg",
            children: [
                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("h1", {
                    className: "mb-6 text-2xl font-bold text-blue-900",
                    children: "Asignar Promoción a Producto"
                }, void 0, false, {
                    fileName: "[project]/app/admin/asignar/page.tsx",
                    lineNumber: 63,
                    columnNumber: 9
                }, this),
                /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("form", {
                    onSubmit: handleSubmit,
                    className: "w-full space-y-6",
                    children: [
                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])(Select, {
                            label: "Producto",
                            value: productoId,
                            onChange: (v)=>setProductoId(v),
                            color: "yellow",
                            placeholder: "Selecciona un producto…",
                            options: productos.map(({ id, nombre })=>({
                                    value: id,
                                    label: nombre
                                }))
                        }, void 0, false, {
                            fileName: "[project]/app/admin/asignar/page.tsx",
                            lineNumber: 67,
                            columnNumber: 11
                        }, this),
                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])(Select, {
                            label: "Promoción",
                            value: promoId,
                            onChange: (v)=>setPromoId(v),
                            color: "blue",
                            placeholder: "Selecciona una promoción…",
                            options: promos.map(({ id, nombre, descripcion })=>({
                                    value: id,
                                    label: `${nombre} — ${descripcion}`
                                }))
                        }, void 0, false, {
                            fileName: "[project]/app/admin/asignar/page.tsx",
                            lineNumber: 77,
                            columnNumber: 11
                        }, this),
                        msg.err && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                            className: "font-semibold text-red-600",
                            children: msg.err
                        }, void 0, false, {
                            fileName: "[project]/app/admin/asignar/page.tsx",
                            lineNumber: 90,
                            columnNumber: 23
                        }, this),
                        msg.ok && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("p", {
                            className: "font-semibold text-green-600",
                            children: msg.ok
                        }, void 0, false, {
                            fileName: "[project]/app/admin/asignar/page.tsx",
                            lineNumber: 91,
                            columnNumber: 23
                        }, this),
                        /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("button", {
                            type: "submit",
                            className: "w-full rounded-lg bg-blue-600 py-2 text-lg font-semibold text-white transition hover:bg-blue-700",
                            children: "Asignar Promoción"
                        }, void 0, false, {
                            fileName: "[project]/app/admin/asignar/page.tsx",
                            lineNumber: 93,
                            columnNumber: 11
                        }, this)
                    ]
                }, void 0, true, {
                    fileName: "[project]/app/admin/asignar/page.tsx",
                    lineNumber: 65,
                    columnNumber: 9
                }, this)
            ]
        }, void 0, true, {
            fileName: "[project]/app/admin/asignar/page.tsx",
            lineNumber: 62,
            columnNumber: 7
        }, this)
    }, void 0, false, {
        fileName: "[project]/app/admin/asignar/page.tsx",
        lineNumber: 61,
        columnNumber: 5
    }, this);
}
_s(AsignarPromosPage, "a+PnBW7FlgYPYDsR5JTuoOTx0N8=");
_c = AsignarPromosPage;
/* --------------------------- Sub-componentes --------------------------- */ const Select = ({ label, value, onChange, placeholder, options, color })=>/*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("div", {
        children: [
            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("label", {
                className: "mb-1 block font-semibold text-gray-800",
                children: label
            }, void 0, false, {
                fileName: "[project]/app/admin/asignar/page.tsx",
                lineNumber: 123,
                columnNumber: 5
            }, this),
            /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("select", {
                value: value,
                onChange: (e)=>onChange(e.target.value === '' ? '' : +e.target.value),
                className: (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$clsx$2f$dist$2f$clsx$2e$mjs__$5b$app$2d$client$5d$__$28$ecmascript$29$__["default"])('w-full rounded px-3 py-2 text-lg border-2', color === 'yellow' ? 'border-yellow-400' : 'border-blue-400', !value && 'text-gray-400'),
                children: [
                    /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("option", {
                        value: "",
                        children: placeholder
                    }, void 0, false, {
                        fileName: "[project]/app/admin/asignar/page.tsx",
                        lineNumber: 133,
                        columnNumber: 7
                    }, this),
                    options.length === 0 && /*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("option", {
                        disabled: true,
                        children: "Sin datos aún"
                    }, void 0, false, {
                        fileName: "[project]/app/admin/asignar/page.tsx",
                        lineNumber: 134,
                        columnNumber: 32
                    }, this),
                    options.map((o)=>/*#__PURE__*/ (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$jsx$2d$dev$2d$runtime$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["jsxDEV"])("option", {
                            value: o.value,
                            children: o.label
                        }, o.value, false, {
                            fileName: "[project]/app/admin/asignar/page.tsx",
                            lineNumber: 136,
                            columnNumber: 9
                        }, this))
                ]
            }, void 0, true, {
                fileName: "[project]/app/admin/asignar/page.tsx",
                lineNumber: 124,
                columnNumber: 5
            }, this)
        ]
    }, void 0, true, {
        fileName: "[project]/app/admin/asignar/page.tsx",
        lineNumber: 122,
        columnNumber: 3
    }, this);
_c1 = Select;
var _c, _c1;
__turbopack_context__.k.register(_c, "AsignarPromosPage");
__turbopack_context__.k.register(_c1, "Select");
if (typeof globalThis.$RefreshHelpers$ === 'object' && globalThis.$RefreshHelpers !== null) {
    __turbopack_context__.k.registerExports(module, globalThis.$RefreshHelpers$);
}
}}),
}]);

//# sourceMappingURL=app_admin_asignar_page_tsx_3596df75._.js.map